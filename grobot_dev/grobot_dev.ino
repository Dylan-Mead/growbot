...
const byte l1 = 4;
//const byte l2 = 3;
const float min_temps[] = {39,41,41,43,44,44,44,44,43,41,39,39};
//const float min_lux[] = {15000, 35000, 50000};
const float max_temp = 101;
const float min_photo_temp = 40;
const float max_humidity = 101;
const byte light_min_hour = 7;
const byte light_max_hour = 19;
...
void envControl(float lux, float temp, float hum, DateTime now){
  ...
  //vent control
  if (vent_on(lux, temp, hum, now)){
    Serial.print(F("VoN."));
    digitalWrite(vent, HIGH);
  } else {
    Serial.println(F("VoFF."));
...
bool light_on(float lux, float temp, float hum, DateTime now){
  bool on = false;
  if ((now.hour() > light_min_hour && now.hour() < light_max_hour) && (temp > min_photo_temp) && (temp < max_temp)){
    on = true;
  }
  else{
    on = false;
...
bool vent_on(float lux, float temp, float hum, DateTime now){
  bool on = false;
...
bool heat_on(float lux, float temp, float hum, DateTime now){
  bool on = false;
