...
void loop() {
  ...
  // write to lcd
  printTimeLcd(now);
  printEnvLcd(temp, humidity);
  printLuxLcd(lux);


  // write to files
  writeToSD(temp, humidity, lux, now);
}
...
void printEnvLcd(float Temp, float Hum){
  lcd.clear();
...
void writeToSD(float Temp, float Hum, float Lux, DateTime Now){
  String dataString = "";
  //dataString += String(Now.year())+'/'+String(Now.month())+'/'+String(Now.day())+',';
  //dataString += String(Now.hour())+':'+String(Now.minute())+':'+String(Now.second())+', ';
  dataString += 'T:'+String(Temp)+', ';
  Serial.print(F("really temp lol?"));
  Serial.println(Temp);
  dataString += 'Rh:'+String(Hum)+', ';
  dataString += 'L:'+String(Lux)+', ';
  ...
  if (dataFile) {
    ...
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.println(F("Logged to SD"));
  }
  else{
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.println(F("log to SD fail"));
    Serial.println(F("Failed to log to SD"));
  }
  delay(2000);

}

void envControl(float Temp, float Hum, float Lux, DateTime Now){
